...
const CharacterListHeading = () => {
	const [visibility, changeVisibility] = useRecoilState(atoms.modalVisibilityState);
	const [filters, changeValues] = useRecoilState(atoms.charactersFilterFormState);
		...
		return (
			<Box marginBottom="20px" d="flex" alignItems="center" isTruncated>
				<Stack isInline d="flex" alignItems="center">
					<Heading as="h1" fontSize={[30, 35]} letterSpacing="2px" color="customGreen">
						Characters
					</Heading>
					{filtersKeys.map(key => {
						if (!filters[key]) {
